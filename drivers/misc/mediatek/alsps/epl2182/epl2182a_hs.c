...
typedef struct _epl_ps_als_factory
{
    ...
    bool ps_cal_finished;
    bool als_cal_finished;
...
//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
#define ALS_CAL_FILE_PATH      "/protect_f/als_cal.dat"
//>2015/03/17-Lever.ni
//>2015/01/26-Lever.ni
...
#if ELAN_WRITE_CALI
static int write_factory_calibration(struct epl2182_priv *epl_data, const char * path,char* ps_data, int ps_cal_len)
{
    struct file *fp_cal;
...
//<2015/01/26-Lever.ni, [V10][Feature][Common][BSP][][]add psensor calibration function
//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
	fp_cal = filp_open(path, O_CREAT|O_RDWR|O_TRUNC, 0775);
//>2015/03/17-Lever.ni 
//>2015/01/26-Lever.ni
	if (IS_ERR(fp_cal))
	{
...
static bool read_factory_calibration(struct epl2182_priv *epl_data, const char * path)
{
	...
	char buffer[100]= {0};
        ...
        if (!strcmp(path, PS_CAL_FILE_PATH)) {
            if(gRawData.ps_als_factory.ps_cal_file_exist == 1) {
                fp = filp_open(path, O_RDWR, S_IRUSR);

        	if (IS_ERR(fp))
        	{
                    APS_ERR("NO PS calibration file(%d)\n", (int)IS_ERR(fp));
                    gRawData.ps_als_factory.ps_cal_file_exist =  0;
        	    return -EINVAL;
        	}
                ...
                APS_LOG("read func %d,%d", ps_hthr, ps_lthr);
	        ...
	        //atomic_set(&epl_data->ps_thd_val_low, epl_data->hw->ps_threshold_low);
                gRawData.ps_als_factory.ps_cal_finished = 1;
            }
        }
        else if (!strcmp(path, ALS_CAL_FILE_PATH)) {
            int als_cali = 0;
            pos = 0;
            fp = filp_open(path, O_RDWR, S_IRUSR);
    	    ...
    	    if (IS_ERR(fp))
    	    {
                APS_ERR("NO ALS calibration file(%d)\n", (int)IS_ERR(fp));
                gRawData.ps_als_factory.als_cal_file_exist = 0;
    	        return -EINVAL;
    	    }
            fs = get_fs();
            set_fs(KERNEL_DS);
            vfs_read(fp, buffer, sizeof(buffer), &pos);
            filp_close(fp, NULL);
            sscanf(buffer, "%d", &als_cali);
            obj->hw->als_cali = als_cali;
            set_fs(fs);
            gRawData.ps_als_factory.als_cal_finished = 1;
        }
	return 0;
}
...
//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
    if(write_factory_calibration(obj, PS_CAL_FILE_PATH,ps_calibration, ps_cal_len) < 0)
//>2015/03/17-Lever.ni 
    {
...
static int elan_epl2182_psensor_enable(struct epl2182_priv *epl_data, int enable)
{
    ...
    if(enable)
    {
        ...
        ret = elan_epl2182_I2C_Write(client,REG_1,W_SINGLE_BYTE,0X02,regdata);
#if ELAN_WRITE_CALI
        if(gRawData.ps_als_factory.ps_cal_finished == 0 &&  gRawData.ps_als_factory.ps_cal_file_exist ==1)
		//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
		    ret=read_factory_calibration(epl_data,PS_CAL_FILE_PATH);
		//>2015/03/17-Lever.ni 	
        APS_LOG("[ELAN epl2182] %s ps_cal_finished = %d\, ps_cal_file_exist = %d\n", __func__, gRawData.ps_als_factory.ps_cal_finished , gRawData.ps_als_factory.ps_cal_file_exist);
#endif
        set_psensor_intr_threshold(epl_data->hw ->ps_threshold_low,epl_data->hw ->ps_threshold_high);
...
static int elan_epl2182_lsensor_enable(struct epl2182_priv *epl_data, int enable)
{
    ...
    if(enable)
    {
    	mutex_lock(&sensor_mutex); //add test 20150320
        regdata = EPL_INT_DISABLE;
        ret = elan_epl2182_I2C_Write(client,REG_9,W_SINGLE_BYTE,0x02, regdata);
        if (gRawData.ps_als_factory.als_cal_finished == 0 && gRawData.ps_als_factory.als_cal_file_exist == 1) { 
            ret = read_factory_calibration(epl_data, ALS_CAL_FILE_PATH);
        }
        if (ret >= 0 && epl_data->hw->als_cali) {
            epl_data->hw->als_cali_flag = 1;
        }
#if ALS_FACTORY_MODE
        ...
        ret = elan_epl2182_I2C_Write(client,REG_7,W_SINGLE_BYTE,0X02,EPL_C_RESET);
        ret = elan_epl2182_I2C_Write(client,REG_7,W_SINGLE_BYTE,0x02,EPL_C_START_RUN);
		mutex_unlock(&sensor_mutex); //add test 20150320
#if ALS_FACTORY_MODE
        if(als_factory_flag == true)
        {
...
static int epl2182_get_als_value(struct epl2182_priv *obj, u16 als)
{
    ...
    int lux = 0;
	//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
    lux = (gRawData.als_ch1_raw * obj->lux_per_count)/1000;
    if (obj->hw->als_cali_flag) {
        lux = lux * 500 * 1000 / (obj->hw->als_cali * obj->lux_per_count);
    }
	//>2015/03/17-Lever.ni 
...
static ssize_t epl2182_store_ps_w_calfile(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    ps_cal_len = sprintf(ps_calibration, "%d,%d", ps_hthr, ps_lthr);
//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
    write_factory_calibration(obj,PS_CAL_FILE_PATH, ps_calibration, ps_cal_len);
//>2015/03/17-Lever.ni 	
	return count;
}
...
static ssize_t epl_show_als_data(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    len += snprintf(buf + len, PAGE_SIZE - len, "%d,%d,%d\n\r", gRawData.als_ch1_raw, obj->hw->als_cali, lux);
    return len;
...
static ssize_t epl_show_ps_cali_savedata(struct device *dev, struct device_attribute *attr, char *buf) {
    ...
    ps_cal_len = sprintf(ps_calibration, "%d,%d", ps_hthr, ps_lthr);
    //<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
    if(write_factory_calibration(epl_data, PS_CAL_FILE_PATH,ps_calibration, ps_cal_len) < 0)
	//>2015/03/17-Lever.ni 
	{
        APS_ERR("[%s] create file error \n", __func__);
        return -EINVAL;
...
//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
static ssize_t epl_show_als_cali_data(struct device *dev, struct device_attribute *attr, char *buf) {
    APS_FUN();
    struct epl2182_priv *obj = epl2182_obj;
    int count = 5;
    char i;
    ...
    obj->hw->als_cali = ch;
    //elan_als_calibration(obj);
    APS_LOG("epl_show_als_cali_data =%d\n", obj->hw->als_cali);
    len += snprintf(buf+len, PAGE_SIZE-len, "%d \r\n", obj->hw->als_cali);
	return len;
}

int run_als_cali_savedata(struct epl2182_priv *epl_data, char *path) {
    char als_cali_data[20] = {0};
    int len = 0;
    len = sprintf(als_cali_data, "%d", epl_data->hw->als_cali);
    if (write_factory_calibration(epl_data,path, als_cali_data, len) < 0) {
        return -1;
    }
    return 0;
}
static ssize_t epl_show_als_cali_savedata(struct device *dev, struct device_attribute *attr, char *buf) {
    int len = 0;
    struct epl2182_priv *obj = epl2182_obj;
    if (run_als_cali_savedata(obj, ALS_CAL_FILE_PATH) < 0) {
        obj->hw->als_cali_flag = 0;
        APS_ERR("als save data fail\n\r");
        return -EINVAL;
    }
    obj->hw->als_cali_flag = 1;
    len = sprintf(buf, "%s\n\r", "success");
    return len;
}
...
//<2015/03/17-Lever.ni [V10][Feature][Common][BSP][][]add light sensor calibration function
static DRIVER_ATTR(als_cali, 0644, epl_show_als_cali_data, NULL);
static DRIVER_ATTR(als_cali_savedata, 0644, epl_show_als_cali_savedata, NULL);
//>2015/03/17-Lever.ni 
    ...
    switch (command)
    {
        ...
        case SENSOR_GET_DATA:
            ...
            else
            {
                ...
                APS_LOG("ps data = %d, stat=%d\n\r", gRawData.ps_raw, gRawData.ps_state);
...
static int epl2182_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    gRawData.ps_als_factory.ps_cal_finished = 0;
    gRawData.ps_als_factory.als_cal_finished = 0;
    gRawData.ps_als_factory.als_cal_file_exist = 1;
#endif

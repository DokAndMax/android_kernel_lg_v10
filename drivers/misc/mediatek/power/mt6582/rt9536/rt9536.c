...
void charging_ic_set_ta_mode(void)
{
    ...
    udelay(1500);
    //>2015/1/20-davidda
    //<2015/6/1-davidda, [V10][Feature][Common][BSP][]change charging current as  HW request
    //<2015/1/26-davidda, [V10][Feature][Common][BSP][]add charging current indicator
    BMT_status.ICharging = 817;
    //>2015/1/26-davidda
    //>2015/6/1-davidda
...
unsigned char rt9536_check_eoc_status(void)
{
    ...
    //<2015/5/4-davidda, [v10][Feature][Common][BSP][]add PGB status check
    mdelay(150);
    pgb_status0 = mt_get_gpio_in(PGB_N);
    mdelay(150);
    pgb_status1 = mt_get_gpio_in(PGB_N);
    //>2015/5/4-davidda
...
//<2015/5/4-davidda, [v10][Feature][Common][BSP][]add PGB status check
extern kal_uint32  ext_charger_status;
...
static int charging_ic_probe(struct platform_device *dev)
{
    ...
    //<2015/5/4-davidda, [v10][Feature][Common][BSP][]add PGB status check
    mt_set_gpio_mode(PGB_N, PGB_MODE);
    mt_set_gpio_dir(PGB_N, PGB_DIR);
    mt_set_gpio_pull_enable(PGB_N, PGB_PULL_ENABLE);
    mt_set_gpio_pull_select(PGB_N, PGB_PULL_SELECT);
    //>2015/5/4-davidda
    udelay(2000);

...
static void hal_rtc_set_spare_fg_value(u16 val)
{
	//RTC_AL_HOU bit8~14
	u16 temp;
//<2015/3/25-davidda, [v10][Feature][Common][BSP][]change the RTC address for FG
#if 0
	rtc_writeif_unlock();
	...
	val = (val & (RTC_NEW_SPARE_FG_MASK >> RTC_NEW_SPARE_FG_SHIFT)) << RTC_NEW_SPARE_FG_SHIFT;
	temp = (temp & RTC_AL_HOU_MASK) | val;
	hal_rtc_xinfo("RTC set FG RTC_AL_DOM value=0x%x\n",temp);
	rtc_write(RTC_AL_DOM, temp);
	rtc_write_trigger();
	hal_rtc_xinfo("RTC get FG RTC_AL_DOM value=0x%x\n",rtc_read(RTC_AL_DOM));
#endif
//>2015/3/25-davidda
}
...
u16 hal_rtc_get_register_status(const char * cmd)
{
	...
	if (!strcmp(cmd, "XTAL")) {
		...
		//RTC_AL_HOU bit8~14
		//<2015/3/17-davidda, [v10][Feature][Common][BSP][]change the RTC address for FG
		//al_hou = rtc_read(RTC_AL_HOU);
		al_hou = rtc_read(RTC_AL_DOM);
		//>2015/3/17-davidda
